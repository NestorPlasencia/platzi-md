...
* **Danvasem** (3) [427427](https://platzi.com/comentario/427427/) 
	...
	**build:local** : Lo que nos importa de este comando es el resultado tanto en **dist/css/app.css** para las hojas de estilo y **dist/js/app.js** para el script, que ambos se agregan en el texto html de server.js, es decir, ambos se ejecutan del lado del cliente.
...
* **Josias Cubillos Gutierrez** (2) [602185](https://platzi.com/comentario/602185/) 
	...
	Error:  
	![Error.png](https://static.platzi.com/media/user_upload/Error-1b6bbaf7-7339-4e92-9118-e9ee7c72babb.jpg)
	
	DevDependencies:  
	![dependencias 1.png](https://static.platzi.com/media/user_upload/dependencias%201-0234fcb5-372d-4e6d-a9dc-ba9bf348da8c.jpg)
...
* **Tonalli López** (2) [546410](https://platzi.com/comentario/546410/) 
	...
	* **Esteban Ciudad** [546410] (3)
		...
		``` 
		    ...
		    npm install react-router@4.3.1 react-router-dom@4.3.1
...
* **David Farinango** (1) [479513](https://platzi.com/comentario/479513/) 
	...
	asi como para la carpeta images, mi duda viene puesto a que no me cargan las images y las tengo dentro de la carpeta src en otra carpeta images, pensé ponerle así:
	``` 
	    app.use('/dist' , express.static('dist'));
	...
	pero igual no me cargan, y no quiero solo mover la carpeta images fuera de src, puesto que me tocarian cambiar las rutas de todos los components, como soluciono para volver static a la ruta images sin sacarla de la carpeta de src  
	Psdt: hice una prueba sacando solo una imagen con la carpeta images fuera de src y le cambie a un componente la ruta y si me funciono así, solo con la imagen que cambie:
	``` 
	    app.use(express.static('dist'));
	    app.use('/images', express.static('images'));
	      ...
	      res.write(`
	        ...
	        <html>
	          ...
	          <body>
	            ...
	            <divid="home-container">${html}</div>
	            <divid="modal-container"></div>
	            <!-- <script src="dist/js/home.js"></script> -->
	            <scriptsrc="http://localhost:9000/js/app.js"></script>
	          </body>
	        </html>
	      `);
	      res.end();
...
Una última cosa antes de terminar con nuestro proyecto, el método render que se maneja en el archivo del cliente es recomendable usarlo solo cuando se tiene render en el lado del cliente, en caso de tener server render como es nuestro caso es mejor usar el método _hydrate_ ya que render siempre esta haciendo un re-render de la aplicación mientras que hydrate va a enlazar los eventos y unirse a lo que ya renderizo el servidor.

### Links:

* [platzi-video-react-router/video.js at master · LeonidasEsteban/platzi-video-react-router · GitHub](https://github.com/LeonidasEsteban/platzi-video-react-router/blob/master/src/pages/containers/video.js)
...
* **Gustavo Pérez Machado** (6) [438432](https://platzi.com/comentario/438432/) 
	...
	o colocar de dentro del comando build:server la compilacion local:
	``` 
	    "build:server":"webpack--env.NODE_ENV=local&&webpack--env.NODE_ENV=local--config./webpack.server.config.js"
...
* **David Daniel Castillo Nava** (1) [376282](https://platzi.com/comentario/376282/) 
	...
	``` 
	    TypeError: Cannot read property 'params' of undefined
	        at Function.mapStateToProps [as mapToProps] (C:\Users\Davecast\Desktop\reactRouter\dist\ssr\app.js:34477:23)
	        at mapToPropsProxy (C:\Users\Davecast\Desktop\reactRouter\dist\ssr\app.js:7782:46)
	        at Function.detectFactoryAndVerify (C:\Users\Davecast\Desktop\reactRouter\dist\ssr\app.js:7791:19)
	        at mapToPropsProxy (C:\Users\Davecast\Desktop\reactRouter\dist\ssr\app.js:7782:46)
	        at handleFirstCall (C:\Users\Davecast\Desktop\reactRouter\dist\ssr\app.js:8258:18)
	        at pureFinalPropsSelector (C:\Users\Davecast\Desktop\reactRouter\dist\ssr\app.js:8306:81)
	        at Object.runComponentSelector [as run] (C:\Users\Davecast\Desktop\reactRouter\dist\ssr\app.js:7178:25)
	        at Connect.initSelector (C:\Users\Davecast\Desktop\reactRouter\dist\ssr\app.js:7330:23)
	        at new Connect (C:\Users\Davecast\Desktop\reactRouter\dist\ssr\app.js:7271:15)
	        at processChild (C:\Users\Davecast\Desktop\reactRouter\node_modules\react-dom\cjs\react-dom-server.node.development.
	...
	``` 
	    ...
	    importVideoPlayer from './video-container'
	    importNotFound from '../components/not-found/404'
	    
	    classVideoextendsComponent{
	      render() {
	...
	``` 
	    ...
	    const logger = ({getState, dispatch}) => (next) => (action) => {
	    	console.group(`Redux-Logger %c${action.type}`, 'color: #2196F3; font-weight: bolder');
	    	console.log(`%cActual State`, 'color: #F44336; font-weight: bold', getState().toJS());
	    	console.log(`%cAction      `, 'color: #FFC107; font-weight: bold', action);
	    	const value = next(action);
	    	console.log(`%cNew State   `, 'color: #4CAF50; font-weight: bold', getState().toJS());
	    	console.groupEnd();
	      return value;
	    }
...
Hola. buenas tardes, al momento de ejecutar npm run watch:server, me genera el siguiente error: con el comando npm run server se ejecuta...
	...
	* **Thony07** [70048] (1)
		...
		``` 
		    ...
		    > platzi-video@1.0.0watch:server C:\Users\Usuario\Documents\react-router\platzi-video-redux-master
		    > babel-watch src/server.js --presets react,es2015,stage-2
		    ...
		    Require stack:
		    - C:\Users\Usuario\Documents\react-router\platzi-video-redux-master\node_modules\babel-watch\babel-watch.js
		        at Function.Module._resolveFilename (internal/modules/cjs/loader.js:772:15)
		        at Function.Module._load (internal/modules/cjs/loader.js:677:27)
		        at Module.require (internal/modules/cjs/loader.js:830:19)
		        at require (internal/modules/cjs/helpers.js:68:18)
		        at Object.<anonymous> (C:\Users\Usuario\Documents\react-router\platzi-video-redux-master\node_modules\babel-watch\babel-watch.js:7:15)
		    ...
		    npm ERR! errno 1
		    npm ERR! platzi-video@1.0.0watch:server: `babel-watch src/server.js --presets react,es2015,stage-2`
		    npm ERR! Exit status 1
		    npm ERR!
		    npm ERR! Failed at the platzi-video@1.0.0watch:server script.
		    npm ERR! This is probably not a problem with npm. There is likely additional logging output above.
		    
		    npm ERR! A complete log of this run can be found in:
		    npm ERR!     C:\Users\Usuario\AppData\Roaming\npm-cache\_logs\2019-10-17T21_09_17_298Z-debug.log
...
Felicidades, ya sabes React Router y como era requisito haber aprobado los cursos de React y Redux significa que ya tienes dominado el stack de React.

Puedes continuar tu aprendizaje viendo el [Curso de Next](https://platzi.com/clases/next-js/), el [Curso de GraphQL](https://platzi.com/clases/graphql/) o el [Curso de React Native](https://platzi.com/clases/react-native/).

Te recomiendo realizar un tutorial, comparte cosas, adéntrate en esta comunidad y enseña para aprender.

Recuerda tomar el examen del curso, darle una review y compartir en redes sociales tu diploma.

### Links:

* [GitHub - LeonidasEsteban/platzi-video-react-router: Proyecto del curso de React Router platzi.com/router](https://github.com/LeonidasEsteban/platzi-video-react-router)
...
* **Luis Arturo Lira Cerda** (1) [605699](https://platzi.com/comentario/605699/) 
	...
	Pero al momento de usar de nuevo build:prod y hacer push a github pages, la carga de imágenes no funciona, sólo funcionan las imágenes con hash que hace url-loader.  
	Pero dejan de cargar las que vienen desde la ruta de src del archivo “api.json”
