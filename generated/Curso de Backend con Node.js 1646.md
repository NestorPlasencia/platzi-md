...
* **Santa Karina Madre de Dios** (1) [912996](https://platzi.com/comentario/912996/) 
	...
	En esta sentí que puedo modularla un poco más con “Ruta - Controlador” pero lo deje así por el momento.
	``` 
	    /*
	    	Year.js
	    */
	      ...
	      if (request.method === 'POST' && request.url === '/echo') {
	        ...
	        request.on('data', (chunk) => {
	          ...
	          if (isoPattern.test(body)) {
	            ...
	            response.end(`Tú día de nacimiento es un ${days[dayBirthday.getDay()]}`);
	          } else {
	        ...
	        response.statusCode = 404;
	        response.end('La página no se encuentra');
	      }
	    }).listen(8080);
	        ...
	        if(req.method === 'POST' && req.url === '/echo'){
	            ...
	            req
	                ...
	                .on('end', () => {
	                    ...
	                    res.end(`Tú naciste un ${days[birth.getDay()]}`)
	                })
	        }else{
...
* **Erick Leao Pérez Medina** (1) [820375](https://platzi.com/comentario/820375/) 
	...
	``` 
	    ...
	    const stringToCamelCase = (str) => {
	      return (
	        str
	          // Removes accents "´`" ex. "Pérez"or"Crème Brulée"
	          .normalize('NFD')
	          .replace(/[\u0300-\u036f]/g, '')
...
* **Erik Ricardo Sánchez Pérez** (2) [784906](https://platzi.com/comentario/784906/) 
	...
	En la clase de [Readable y Writeable streams](https://platzi.com/clases/1646-backend-nodejs/22019-readable-y-writable-streams/) menciona que el `process.stdout` lo llama `console.log` por debajo, esto me ha parecido muy interesante entonces decidí a probar a hacer mi propio console.log.
...
* **carlosextra1** (1) [1088410](https://platzi.com/comentario/1088410/) 
	...
	* **carlosextra1** [1088410] (1)
		...
		```
		    ...
		    [info number] => 30
		    [info json] => {"Nombre":"Carlos Ramírez"}
...
* **Jesús Miguel Quinto Teran** (1) [827287](https://platzi.com/comentario/827287/) 
	...
	[customConsole.info](http://customConsole.info)(‘Este es un mensaje de Informacion’);
...
* **David Behar** (1) [786909](https://platzi.com/comentario/786909/) 
	...
	``` 
	    ...
	    fetch('https://randomuser.me/api/?results=5000')
	      ...
	      .then((myJson) => {
	        consoleFile.log('Bienvenido a la oficina del trabajo, le podemos buscar un empleo, si es quenoestá juvilado');
	        myJson.results.map((user) => {
	          ...
	          if (age <= 60) {
	            ...
	            const message = `${fullName}, usted tiene ${age} años, ya recibe una pensión, diviértase, ¡no todo enla vida es trabajar!`;
...
Tengo problemas para installar el npx mrm lint-staged, ando programando en windows y el error que me sale es el siguiente: Agradeceria d...
	...
	* **glrodasz** [74160] (1)
		...
		Yo revisaría dos cosas:
		  ...
		  2. Usar mejor la version de Ubuntu para Windows y trabajar desde allí para evitar problemas de compatibilidad: <https://ubuntu.com/tutorials/tutorial-ubuntu-on-windows#2-requirements>
	...
	``` 
	    ...
	    app.get('/:amo', (req, res) => {
	        ...
	        if ((((amo % 4 == 0)
	            ...
	            res.send(`El año ${amo} NO es bisiesto`);
	    });
...
¿Qué significa eso de quemar un puerto? ¿Cuál es el riesgo? ¿Alguien me podría explicar? Por favor 😦 ¿Hay alguna documentación sobre eso?
	...
	* **Manuel Ojeda** [68916] (4)
		...
		Esa parte de los **:3000** es e; puerto, lo adecuado sería que en un .env pongan el puerto
		``` 
		    PORT=3000
		...
		Por lo que si lo hacemos así desde express podemos:
		``` 
		    axios({
...
* **Chavez** (1) [884584](https://platzi.com/comentario/884584/) 
	...
	* **SandroMartinez** [884584] (1)
		...
		y agregando “@babel/transform-runtime”, en plugins dentro de mi archivo .babelrc
		``` 
		    {
		      "presets": [
		        "@babel/preset-env",
...
buenas tardes companeros. Pregunta, Porque la function moviesApi recibe el app como parametro?
	...
	* **Marco Antonio Benites Espinoza** [82657] (1)
		...
		Con la finalidad de que en el archivo routes/movies.js pueda mapear los endpoints mediante la variable app ( const app = express() ) mediante el router “/api/movies”, esta variable se le debe pasar como parametro en el metodo moviesApi:
		``` 
		    functionmoviesApi(app) {
		        ...
		        app.use("/api/movies", router);
...
Hola, el siguiente código : app.use("/api/movies", router); - le dice a router que la ruta a la que debe responder es “/api/mov...
	...
	* **Erik Ochoa (Platzi)** [71364] (2)
		...
		Recuerda que puedes usar tres “backticks(```)” para dar formato de código en la caja de comentarios o simplemente dar click en “insertar código”.
	...
	* **Joe Harry Mancera Monroy** [1015579] (1)
		...
		``` 
		    router.get('/:movieId', async (req, res, next) => {
		            try {
		                const movies = await Promise.resolve(moviesMock[0]) <= aquí
		                res.status(200).json({
		                    data: movies,
...
Puede que esté preguntando algo tonto pero, ¿Cómo funciona eso del alias que hizo en el minuto 7:18? Me gustaría saber más de ello, y si ...
	...
	* **Manuel Ojeda** [68952] (2)
		...
		sería lo mismo solo que ahora puedes acceder a ese valor con movies.
...
* [Collection Methods — MongoDB Manual](https://docs.mongodb.com/manual/reference/method/js-collection/)
...
Cuando pense que todo estaba bien, me salio un error en los servicios que se debe enviar el ID, me sale el siguiente error Error: Argumen...
	...
	* **Luis Miguel gonzalez** [77577] (1)
		...
		y había que utilizar las llaves
		``` 
		    const { moviesId } = req.param
...
Tengo el siguiente problema al intentar realizar una petición, me dice Invalid Connetion String. Comparto mi codigo para ver si alguien m...
	...
	* **Luis Miguel gonzalez** [77566] (1)
		...
		Error: MongoParseError: Ports not accepted with ‘mongodb+srv’ URIs  
...
Por que el los parametros de las funciones las coloca dentro de llaves y no solamente las coloca como parametros unicos?? Ejemplo el pone...
	...
	* **Juan David Castro (Platzi)** [71919] (4)
		...
		Ahora el negocio crece y tu función puede alimentar más animales:
		``` 
		...
		Ahora al enviar tus argumentos debes saber exactamente en qué posición va cada animal:
		``` 
		    const perrito = new Dog("Patricio");
		...
		Así es más fácil equivocarte. Si tienes muy bien tus nombres de variables pos todo va a estar bien (como en este caso). Pero imagínate un caso real: tienes 20 archivos diferentes y alguien del equipo añade un nuevo animal todos los días.
		
		Al usar argumentos con nombres te evitas muchos problemas. No tienes que recordar la posición de cada argumento, sino solo su nombre.
		``` 
...
* **edwintrumpet** (4) [789784](https://platzi.com/comentario/789784/) 
	...
	**Dato:** El orden en el que ponemos los middlewares en este caso importa muchísimo.
	...
	El siguiente middleware es _logErrors_ que igual tiene un callback _next()_ que le envía el error al siguiente middleware.
...
Los errores que ocurren de manera síncrona dentro un manejador de rutas o un _middleware_ no requieren trabajo extra. Si un código síncrono lanza un error Express automáticamente capturará el error. Por ejemplo:
``` 
    app.get("/", function(req, res) {
...
Para errores que se retornan desde funciones asíncronas invocadas desde un manejador de ruta o un _middleware_ , es necesario pasar el error como argumento a la función `next()`, de esta manera Express capturará el error y lo procesará. Por ejemplo:
``` 
...
Es responsabilidad de nosotros capturar errores que puedan ocurrir en código asíncrono invocado desde un manejador de ruta o _middleware_ para que Express lo procese. Por ejemplo:
``` 
    app.get("/", function(req, res, next) {
      setTimeout(function() {
        try {
...
A continuación te compartiré una lista de los _middlewares_ más populares en Express.
...
_Middleware_ para habilitar CORS (Cross-origin resource sharing) en nuestras rutas o aplicación. <http://expressjs.com/en/resources/middleware/cors.html>
...
Passport es un _middleware_ que nos permite establecer diferentes estrategias de autenticación a nuestras aplicaciones. <https://github.com/jaredhanson/passport>

> Puedes encontrar más _middlewares_ populares en el siguiente enlace: <http://expressjs.com/en/resources/middleware.html>
...
* [GitHub - visionmedia/supertest: 🕷Super-agent driven library for testing node.js HTTP servers using a fluent API.](https://github.com/visionmedia/supertest)
...
* [Curso de Buenas Prácticas para la Escritura de Código](https://platzi.com/cursos/mejor-codigo/)
...
Para ello se debe acceder al servidor remoto:
  ...
  3. Usar valores y servicios diferentes para cada ambiente, esto quiere decir que las credenciales de desarrollo, _staging_ y producción deben ser completamente diferente.
...
> Como lo hemos dicho antes no se debe hacer _commit_ del archivo `.env` y este debe estar en el `.gitignore`, ademas se recomienda manejar solo un archivo `.env`. Más información: <https://github.com/motdotla/dotenv#faq>
...
### Habilitar CORS para todos los _request_ (No recomendado en producción)
``` 
    const express = require("express");
    const cors = require("cors");
    const app = express();
...
### Habilitar CORS para los _request_ específicos de un cliente (Recomendado para producción)
``` 
    const express = require("express");
    const cors = require("cors");
    const app = express();
...
Más información sobre el _middleware_ CORS en <https://expressjs.com/en/resources/middleware/cors.html>
...
Dependiendo el servicio de _hosting_ que estemos usando puede ofrecernos o no una instalación de certificados de seguridad SSL/TLS que pueden tener algún costo. Sin embargo existen servicios como [Let’s Encrypt](https://letsencrypt.org/) que permiten la instalación de este certificado completamente gratis. Servicios como Now y Heroku ofrecen HTTPS por defecto.
...
* [Curso de Autenticación con Passport | Materiales](https://platzi.com/clases/passport/)

* [Curso de Integración Backend con Frontend | Materiales](https://platzi.com/clases/bff/)
...
Mi duda son todas las rutas originales de Express. ¿porque no se agregaron al archivo de configuración de now.json? y en su lugar se colo...
	...
	* **Erik Ochoa (Platzi)** [68391] (2)
		...
		**Segunda pregunta** : Claro, siempre puedes hacer cambios en tu proyecto y volver a hacer deploy con el comando `now`, por default las nuevas versiones de Now no te cambian la URL. Si tienes una alías has deploy con este comando para que lo mantenga `now alias $(now) tu-alias.now.sh`.
