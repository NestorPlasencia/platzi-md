...
* [Tensiones entre atributos - Google Sheets](https://docs.google.com/spreadsheets/d/1imnq3NDIxI6j5dn0o40Dej6-NKslxLG8556Un7kGwMg/edit?usp=sharing)
...
* [PdAS - Proyecto:  La máquina del tiempo - Google Slides](https://drive.google.com/open?id=1maSgvM4mXn7bldGlYEBHnOC_Lm9PY9B7CnfKXvg_lmg)
...
* [PdAS - 2 - Patrones de Arquitectura - Google Slides](https://drive.google.com/open?id=1fP3vMpxz36MqI7MBGnmB90_FvtOQx6eXda1RzTu7jkU)
...
* **marczg21** (1)
	...
	* **Claudio Lepin** (7)
		...
		Además deberás tener una API Gateway para orquestar todos estos microservicios y administrarlos de la mejor forma para TU aplicación.
...
* **John Cardenas** (2)
	...
	* **SistemasCBC** (3)
		...
		No se recomienda para pequeños equipos ni para aplicaciones simple. Amenos que se tenga muy bien entrenado lo que es DDD.
		
		Una de las grandes desventaja de DDD, es su implementacion.  
		Puede ser muy costo, ya que puede tardar hasta 6 meses en entrenar un equipo, realizar modelos, pactar un lenguaje comun y mas . (6 meses sin hacer update, push, etc de la aplicacion).  
		Pero dependiendo de la aplicacion es justificable.
		
		DDD busca resolver problemas grandes.  
		Un claro ejemplo puede ser un banco.  
		En el cual realizas la aplicacion a la logica del negocio.  
		Vos como desarrollador no lo podes realizar solo(En realidad si) pero se recomienda hacerlo en conjunto con los expertos del dominio.  
		En este caso un banquero, el es el experto del dominio que sabe como funciona el negocio.
		
		Del dominio se sacan sub-dominios (bounded contex) para dividir los problemas a resolver (dividie y venceras) Cada boundend contex va estar una problematica especifica. (User, Payment, Reports, etc).  
		En el cual se hacen charlas de varios dias o puede ser sermanas para pactar un lenguaje en comun "ubiquitous language ".  
		Tener en cuenta que cada Palabra puede representar diferentes cosas en cada Boundend Contex.  
		Muchas veces se piensa que cada Boundend Contex, es como si fuera una empresa separada en el cual User puede representar algo en User y en otro boundend contex signifique otra cosa.  
		Cada Bounded Contex puede ser desarrollada en cualquier lenguaje, tecnologia, arquitectura etc. (dependiendo de su caso de uso obviamente, va ser mejor una cosa que otra).  
		Pero tambien al estar separado, se puede migrar facil mente a otra tecnologia sin que afecte al resto de boundend contex.  
		Es recomdable dividir en:  
		Aplicacion  
		Dominio  
		Infraestructura.
...
* [PdAS - 3 - Diseño de una Arquitectura - Google Slides](https://docs.google.com/presentation/d/1SrNVT_j8De1TipH8GgGt-aq8vctd55wyinuEEO--UcE/edit?usp=sharing)
...
* **Lautaro Cabral** (1)
	...
	* **mauri_costanzo** (2)
		...
		**¿Por qué?** debido a que si mañana se nos ocurre darle condiciones a al precio del producto (Por ejemplo: si nos dan un requisito nuevo que dice: _si el usuario en mayorista el precio se debería reducir_ )
		
		De esta manera **no deberíamos tener que cambiar cambiar nada** en nuestro código donde se esté usando tal funcion que devuelve precio porque la lógica la habriamos implementado en la misma. **En el caso contrario donde se estuviera usando directamente la propiedad si deberíamos cambiar todo**.
...
* **David Andrade Morales** (11)
	...
	**Eficiencia de ejecución: **Tenemos eventos, trabajamos con tácticas para controlar la eficiencia para dar una respuesta dentro del tiempo esperado. Las tácticas son 3 grandes familias:
	...
	Gestión de recursos: Cuántos ponemos disponibles y cómo hacemos para que estén disponibles cuando se necesitan ¿Cómo aprovechamos nuestros recursos dentro de nuestros propios recursos?
	...
	Arbitraje de recursos: En caso de conflicto de eventos requiriendo los mismos recursos saber cómo y a quien administrar el recurso por orden de prioridad.
...
si tengo un problema de lentitud en mi aplicación esto podría deberse a la arquitectura de desarrollo ?
	...
	* **mauri_costanzo** (1)
		...
		** El buen uso de ptrarones de diseño** te va a ayudar. Aquí te enumero algunos de los niveles en donde podrías amplicarlos.  
		(Ten en cuenta que los patrones de diseño son formas de solucionar algo que usualmente ya están verificadas por otros colegas o mejor dicho por la comunidad. La tareas difícil es saber cuales de esos patrones deberías usar un tu caso)
...
* **Antonio Rafael González Ferrer** (6)
	...
	**-Traza de auditoría.** Sepamos exactamente todos los pasos que dan nuestros usuarios como para que, cuándo detectamos al atacante, podamos volver a un estado conocido y luego ejecutar todas las acciones de nuestros usuarios ignorando lo que hizo el atacante.
...
* **Antonio Rafael González Ferrer** (8)
	...
	**• Captura y reproducción.** Capturar la comunicación, es decir saber qué es lo que le están diciendo a la aplicación y luego usar esa misma comunicación en un test de prueba.  
	**• Separar interfaz de implementación.** Cuando separamos la interfaz de la implementación podemos hacer pruebas remplazando la implementación con una implementación controlada y evaluar si esa implementación está recibiendo los mensajes que deseamos que reciba.  
	**• Acceso exclusivo para pruebas.** Cómo hacer cuándo tenemos una funcionalidad y hay cierta parte que no podemos controlar desde fuera de la aplicación. Entonces si queremos hacer una prueba desde fuera de la aplicación podemos darnos acceso a cierta parte como contexto de testing.
...
* **JAVIER STIVEN MURILLO CORREA** (3)
	...
	[Backlog](http://agilismoatwork.blogspot.com/2014/05/backlog-todo-el-trabajo-pendiente-del.html) El Backlog es una lista ordenada de todo el trabajo pendiente.
...
* [PdAS - 4 - Modelado y Documentación de Arquitectura - Google Slides](https://drive.google.com/open?id=1tJsbh1b4w-JPzvCAJm-FB3ZXUg2DIMHtU2lWgDErCLw)
