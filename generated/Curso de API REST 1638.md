...
* **Walter Jesus Santiago Gonzalez** (1)
...
## 0020. Qué es una API y para qué sirve
...
* **Hugo Rodriguez** (3)
	...
	* **jarios017** (1)
		...
		Se presento el mismo error inicial. Con la solución planteado funciono correctamente.
...
## 0030. Qué es y cómo funciona el protocolo HTTP
...
## 0040. ¿Qué significa REST y ¿qué es una API RESTful
...
* **john jairo lopez ramirez** (9)
	...
	* **Manuel  Alfonso Illidge ramirez** (4)
	...
	* **Maria Paula Rodriguez Escobar** (17)
...
Y por ejemplo, en qué tipo de caso la interacción se pone compleja ? Y supongo usaríamos soap ?
	...
	* **Mauro Chojrin** (2)
		...
		Piensa en algún cálculo que involucre mucha lógica.
...
## 0050. Cómo realizar una petición REST e interpretar sus resultados
...
* **Emilio Ian Camacho Mejia** (1)
	...
	* **msantim** (3)
		...
		Si puedes subir un enlace de tu archivo, podriamos ayudarte mejor 😄

	* **JeanCarlosRodriguez** (3)
...
## 0060. Exponer datos a través de HTTP GET
...
* **Marco Antonio González Arellano** (7)
...
## 0070. Exponer un recurso en particular a través de HTTP GET
...
* **Aarón Anibal Ramirez Herrera** (4)
...
* **Cristhian Edwin Ortiz Mercado** (1)
...
## 0080. Incorporar datos a través de HTTP POST
...
* **Daniel Boullon** (1)
	...
	Sigo con el codigo en Python 3… agregue POST:
	``` 
	    from flask import Flask, jsonify, make_response
...
* **nova12** (1)
Tengo una pregunta , en un caso real, si tengo una aplicación web y conecto mi pagina a través de ajax , lo aria con la url del archivo ...

	* **jemc300x** (1)

		
		si por ejemplo tienes un archivo llamado <http://tuweb.com/api.php> y este devuelve un json, fácilmente puedes consumirlo a través de ajax.
...
## 0090. Modificar datos a través de HTTP PUT
...
* **Christian David Sánchez** (3)
	...
	* **william andres rodriguez borja** (5)
...
* **Aarón Anibal Ramirez Herrera** (2)
...
* **Luis Abraham Rosas Barajas** (1)
	...
	* **Jaime Ortiz** (1)
		...
		Es muy recomendable hablar con los asesores de platzi para que le asignen rutas de aprendizaje y así llegar a los cursos deseados luego de haber realizado los que son prerrequisitos.
...
* **cesarcubillos** (1)
¿No es necesario recibir el /1 al final de la URL? ¿O cómo es que la variable PHP de $resourceId se estableció con ese /1?

	* **Moisés Cedeño** (1)

		
		Sí, es necesario. Eso se hace en la línea 47, si estamos manejando el mismo identado.
		``` 
		    $resourceId = array_key_exists('resource_id', $_GET ) ? $_GET['resource_id'] : '';
...
* **Emerson Cedeño Salazar** (1)
yo supongo que para implementar actualizaciones ‘parciales’ habrá que hacer una consulta previa al registro bajo modificación, comparar v...

* **platzerito02112019 platzerito02112019** (1)
Todo funcionaba Bien hasta que llegue al método ‘POST’ Y ‘PUT’ y me salio este error: (estoy con trabajando con windows) <h1>POST...

	* **Mauro Chojrin** (1)

		
		El problema está en las comillas. Prueba con:
		
		curl -X ‘PUT’ “<http://localhost:8000/books/1>” -d ‘{“titulo”: “Nuevo Libro”, “id_autor”: 1, “id_genero”: 2}’

## 0100. Eliminar datos a través de HTTP DELETE
...
* [Meet Google Drive – One place for all your files](https://drive.google.com/open?id=1RerWOWXTFI-Zyq6_cf1NsDfvhknN4Md7)
...
* **mariorm1106** (1)
	...
	Autenticación or html, O: tan fácil como [user:password@platzi.com](mailto:user:password@platzi.com).  
...
* **MisaMagna** (1)
	...
	* **Joshua Nathanael Saucedo Uriarte** (6)

		
		la **s** en **https** es porque la comunicación es encriptada utilizando Transport Layer Security (TLS) o Secure Sockets Layer (SSL). Es http con una capa extra de encriptación y eso solo es a nivel comunicacion, no a nivel de Autenticación de usuario
	...
	* **Emerson Cedeño Salazar** (3)
		...
		no obstante, aún así podría aprovecharse ( **HTTPS** ) para agregarle seguridad a este método de autenticación mediante el uso del `CURLOPT_USERPWD` del `curl_sepopt` en PHP, ¿correcto?
...
* **Franklin Rosas** (7)
	...
	Para más información sobre el tema puedes visitar este [blog de kaspersky](https://latam.kaspersky.com/blog/que-es-un-hash-y-como-funciona/2806/)

* **Walter Jesus Santiago Gonzalez** (6)
...
* **SistemasCBC** (1)
	...
	* **Juan Sebastián Joya Rodríguez** (2)
		...
		Tu te acurdas del codigo enigma? para decodificar el mensaje habia que encontrar una “Semilla”, bueno, pinesa en el timeStamp como esa Semilla
...
* **Emerson Cedeño Salazar** (1)
luego de leer acerca del HMAC el UID no es, conceptualmente hablando, un parámetro necesario de este método de autenticación, ¿correcto?

* **operez12** (1)
En este ejemplo llevándolo a la realidad el mensaje que se comparte entre las dos partes seria una llave pública?

	* **Emerson Cedeño Salazar** (1)

		
		“el mensaje” _(la ‘palabra secreta’, o semilla -seed)_ no necesariamente es una “llave pública”, más bien se refiere _(a como mencionó el profe)_ a una cadena alfanumérica aleatoria que ambas partes deben conocer.

## 0130. Autenticación vía Access Tokens
...
* **albertodanielcastillomendoza** (4)
...
* **luis-salazar373** (1)
Yo estoy teniendo problemas debido a que al hacer la petición de libros y enviar el X-Token que me devuelve el servidor… No obtengo ningu...

* **esteban-chamba-jimenez** (1)
Hola, me queda entendido todo, muy claro, pero tengo una pregunta tonta, y es que yo no entiendo la diferencia con una función normal que...
...
* **Moisés Cedeño** (1)
**¿Cuántos actores intervienen en la comunicación vía Acces Tokens? ** ¿2 (servidor de recursos y el de autenticación)? ¿O 3 (usuario, se...
...
## 0140. Manejo de errores de un servicio REST
...
* **Alex Pacheco** (2)
	...
	* **Franklin Rosas** (3)
		...
		Es un niño con una bomba de chicle que se reventó en su boda… me parece que es un símil a cuando explota las REST API y no sabes que hacer.
...
* **Michael Emir Reynosa Beltrán** (1)
...
* [Qué es AJAX y como funciona - YouTube](https://www.youtube.com/watch?v=_ybgWmSCAu8)
...
* **Franklin Rosas** (5)
	...
	Si quieres leer un poco más al respecto encontraras información en: [https://www.arweb.com/chucherias/¿que-es-bootstrap-y-como-funciona-en-el-diseno-web/](https://www.arweb.com/chucherias/%C2%BFque-es-bootstrap-y-como-funciona-en-el-diseno-web/)  
	y aquí también:  
...
* **Rafael Carrillo** (2)
	...
	* **Gabriel De Andrade (Platzi)** (4)
		...
		Algunas personas prefieren tener la explicación, otras prefieren ir directo al grano 🤷. De todas maneras te puedes saltar esa parte de la clase y descargar el archivo desde la sección de enlaces 😄
...
* **Ivan Alexis Carbajal Sandin** (2)

	
	Me perdí, como hizo que el servidor tuviera el HTML después de ser levantado??

	* **Claudio Jesus Vázquez Villanueva** (6)

		
		El servidor al no tener un punto de entrada lo que hace es Buscar automáticamente un Index. Es como declarar que abriera implícitamente el index.
...
* **MisaMagna** (4)
	...
	* **Emerson Cedeño Salazar** (8)

		
		créeme que de primera pensé lo mismo, mas como dice el profe: por temas de **simplicidad** está utilizando esta librería; y tomando en consideración que el curso es de **API REST** y ~~no de javascript~~ _(que su implementación con vanilla conllevaría a más líneas de código y podría desviar el tema núcleo del curso…)_ , entonces me parece válido …
...
* **albertodanielcastillomendoza** (1)
	...
	* **albertodanielcastillomendoza** (2)

		
		Si alguien le pasa lo mismo, logre resolverlo.  
		En el $.ajax después de la url le agregue esto:  
		‘url’: ‘<http://localhost:8000/books>’,  
		‘dataType’ : ‘json’,
		
		Quedando así y después ya todo igual. Mas aparte el profesor modifica algo del router.php en el siguiente video 😃
...
* **sebastianjoshua** (1)
Aquí el profesor levanta el servidor con: php -S localhost:8000 router.php No? o lo hace al igual que en la clase anterior c...

	* **Pablo_** (1)

		
		Me pasó exactamente lo mismo, así que intenté levantando el servidor _**php -S localhost:8000 router.php**_ y funcionó.
...
Tengo una pregunta: existe alguna otra alternativa para implementar Ajax que no sea jquery ??. He escuchado comentarios sobre que jquery ...
	...
	* **Juan Carlos Pinzón** (1)
		...
		Por supuesto, puedes usar JavaScript puro para eso tenemos **Fetch** y pues funciona con promesas. JavaScript moderno, ten en cuenta que quizá no funcione en todos los navegadores, lo puedes transpilar. O hay una librería que he visto que está un poco de moda se llama Axios.
...
* **Cristian Pisco** (1)
	...
	* **abrahamaquinoe** (1)
		...
		– en file_get_contents() pon la ruta de tu archivo index.html (si todo está en una sola carpeta solo pones ‘index.html’)
		
		Ejecuta el servidor:  
		php -S localhost:8000 router.php
		
		Si la ruta <http://localhost:8000/books> lo ejecutas en el navegador, deberías obtener el json.
...
* **Christian David Sánchez** (2)
	...
	Que espectacular, no conocía NOW para el deployment 😮

* **MisaMagna** (2)

	
	Esto de [now.sh](http://now.sh) me ha parecido espectacular, siempre me pregunte como podía yo hacer un deployment de una aplicación front, así que muchas gracias!
...
## 0190. 7 Buenas prácticas del diseño de APIs RESTful
...
* **Fabio Andres Arango Grajales** (3)

	
	Gracias platzi justo lo que necesito en mi trabajo,

* **Michael Emir Reynosa Beltrán** (2)
...
* **David Santiago González Ramos** (1)
	...
	* **Kevin Javier Morales (Platzi)** (1)
		...
		Cuéntanos que fue lo que más te gusto del curso 😃
...
* **annaduarte** (1)
	...
	* **Moisés Cedeño** (1)
		...
		¿Qué acción quieres hacer con take-joboffert?
		
		Recuerda que tienes 4 métodos básicos: GET, POST, PUT, DELETE.
		
		  1. Si al ejecutar esa acción quieres crear un nuevo registro de una persona que tomó un trabajo, debería usar POST.
		
		
		
		Ejemplo: /employees/take-joboffert/1.
		
		Usas el método POST, donde 1 es el id de la persona que está tomando el trabajo.
		
		  1. Si deseas modificar la vacante de trabajo y decir que el atributo inscritos ahora es 1, deberías usar PUT.
		
		
		
		Ejemplo: /employees/take-joboffert/10
		
		Usas el método PUT, donde 10 es el id de la vacante a la cuál ahora le vas a inscribir a 1 persona.
		
		  1. Si deseas eliminar la vacante de trabajo porque una persona la está tomando, debes usar DELETE.
		
		
		
		Ejemplo: /employees/take-joboffert/1
		
		Usas el método DELETE vacante con id 1 porque la están tomando.
